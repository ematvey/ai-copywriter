...
def _concat_fw_bw_state(fw_state, bw_state):
  if isinstance(fw_state, LSTMStateTuple):
    ...
    return state
        ...
        len(fw_state) == len(bw_state)):
    # multilayer
    state = tuple(_concat_fw_bw_state(fw, bw) for fw, bw in zip(fw_state, bw_state))
    return state
  else:
    raise ValueError('unknown state type: {}'.format((fw_state, bw_state)))

def bidirectional_rnn_layered(cell, layers, inputs_embedded, input_lengths,
                              time_major=False, scope=None):
  """Bidirecional RNN with fw/bw concatenation between layers"""
  layer_outputs = []
  layer_states = []
  inputs = inputs_embedded
  with tf.variable_scope(scope or "birnn"):
    for i in range(layers):
      with tf.variable_scope("layer%d" % i) as scope:
        ...
        with tf.variable_scope("concat"):
          state = _concat_fw_bw_state(fw_state, bw_state)
